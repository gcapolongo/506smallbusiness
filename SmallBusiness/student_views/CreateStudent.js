...
export default class CreateStudent extends React.Component {
  ...
  CreateAccount() {
    ...
    if (this.state.password !== this.state.cfpassword) {
      ...
      auth
        ...
        .catch((error) => {
          //error code example: auth/weak-password
          //dont see a need to use this
   ...
   */
  writeUserData(uid) {
    //I want to push the uid of the user
    var myRef = database.ref("Users/Customers").child(uid);
    //console.log("PRINT UID IN WRITE USER METHOD: " + uid);
